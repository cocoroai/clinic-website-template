# images/dna/ 写真クレジット・ライセンスメモ

すべて **Unsplash License**（商用利用可・クレジット表記不要・再配布のみ不可）。
規約: https://unsplash.com/license
取得日: 2026-07-06 ／ 取得サイズ: 長辺1600px・品質78（imgix パラメータ `w=1600&q=78`）
...
| counseling.jpg | 医師と患者のカウンセリング風景 | Vitaly Gariev | https://unsplash.com/photos/doctor-consulting-with-patient-at-desk-GBcLdqOBbXE |

## 注意
- Unsplash+（premium_photo / plus.unsplash.com 配信）の画像は**不使用**（有料ライセンスのため除外済み）
- 旧 `images/` 直下の写真（hero-image.jpg 等）は他ページ（services.html 等）が参照しているため削除・変更しない
- index.html は本フォルダ（images/dna/）の写真のみを参照する
